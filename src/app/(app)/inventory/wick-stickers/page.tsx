...
export default async function WickStickerPage() {
  ...
  let ws = await prisma.wickSticker.findUnique({ where: { userId: user.id } });
  if (!ws) ws = await prisma.wickSticker.create({ data: { userId: user.id } });
  const low = ws.stockQty < LOW_STOCK.wickStickerQty;
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">Wick stickers</h1>
      <p className="text-xs text-gray-500">
        One sticker per wick. Quantities follow the wick rule for each jar size.
      </p>
