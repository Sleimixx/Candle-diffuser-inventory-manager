...
export async function runProduction(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    const r = await tx.candleRecipe.findUnique({
      where: { id: recipeId },
      include: { waxes: { include: { wax: true } }, scents: { include: { scent: true } } },
    });
    if (!r) throw new Error("Recipe not found");

    const { type: wickType, qty: wickPer } = wickRuleFor(r.jarSize);
    const [wick, ws, jar, sticker] = await Promise.all([
      tx.wick.findUnique({ where: { type: wickType } }),
      tx.wickSticker.findFirst(),
      tx.jar.findUnique({ where: { size_color: { size: r.jarSize, color: r.color } } }),
      tx.sticker.findUnique({ where: { size_color: { size: r.jarSize, color: r.color } } }),
    ]);
    if (!wick || !ws || !jar || !sticker) throw new Error("Missing inventory rows.");

    // Stock checks
    const wickNeed = wickPer * quantity;
    const wsNeed   = wickPer * quantity;
    if (wick.stockQty    < wickNeed) throw new Error(`Not enough ${wickType} wicks (need ${wickNeed}, have ${wick.stockQty}).`);
    ...
    const cogs    = waxCost + scnCost
                  + wick.unitCost * wickPer
                  + ws.unitCost   * wickPer
                  + jar.unitCost
                  + sticker.unitCost;

    // Deduct
    await tx.wick.update({ where: { id: wick.id },       data: { stockQty: { decrement: wickNeed } } });
    ...
    await tx.productionRun.create({
      data: { recipeId, quantity, cogsPerUnit: cogs },
    });
  });
